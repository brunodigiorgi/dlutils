...
class RNNTest:
    def __init__(self, dataset, model, batch_size, logger,
                 nfolds=5, proportion=1, update_interval=120, log_epochs=1, max_epochs=1000, model_path='models/'):
    ...
    def test(self):
        ...
        for i_fold, (train_set, test_set) in enumerate(self.k_fold):
            ...
            di_train = DatasetIterator(self.dataset, list_train_seq, self.conf['batch_size'])
            di_test = DatasetIterator(self.dataset, list_test_seq, self.conf['batch_size'])
